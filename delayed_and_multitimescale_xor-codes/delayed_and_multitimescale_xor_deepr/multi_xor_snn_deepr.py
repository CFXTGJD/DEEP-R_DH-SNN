...
parser.add_argument('--use_even_mask', type=bool, default=True, help='Use even mask')
...
class Dense_test_1layer(nn.Module):  # DH-SFNNs with 1 layer
    def __init__(self, input_size, hidden_dims, output_dim):
        ...
        self.dense_1 = spike_dense_test_denri_wotanh_R_xor(input_size,hidden_dims,
                                    tau_ninitializer = 'uniform',low_n = 2,high_n = 6,vth= 1,dt = 1,branch = args.branch,device=device,bias =False,use_even_mask=args.use_even_mask)
...
def train(epochs,optimizer,scheduler=None):
    ...
    for epoch in range(epochs):
        ...
        model.dense_1.dense.modify_theta(lr,gdnoise,l1)
        target_nb_connection = (model.dense_1.input_dim+model.dense_1.pad)*model.dense_1.output_dim
        model.dense_1.dense.rewiring(target_nb_connection, epsilon=1e-12)
...
base_params = [                    
                    ...
                    model.dense_1.dense.th_1,
                    model.dense_1.dense.W_1,
...
wandb.init(project=f"multi_xor")
wandb.watch(model)
wandb.run.name = f"DEEPR, branch={args.branch}, use_even_mask={args.use_even_mask}"
